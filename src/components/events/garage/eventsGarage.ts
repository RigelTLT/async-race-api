...
export function chekerUpdate(event: Event) {
  ...
  const updateElelentCar = document.querySelector(`.garage-list__element[data-id="${id}"]`) as HTMLElement;
  const valueNameCar = (updateElelentCar.childNodes[1] as HTMLElement).innerHTML;
  const valueColorCar = ((updateElelentCar.childNodes[2].firstChild as HTMLElement).firstChild as HTMLElement).style.fill;
...
export async function changePageNumber(event: Event) {
  ...
  const next = document.querySelector('.next') as HTMLInputElement;
  const prev = document.querySelector('.prev') as HTMLInputElement;
...
export async function startCar(id: number) {
  ...
  const updateCarEl = document.querySelector(`.garage-list__element[data-id="${id}"]`) as HTMLElement;
  ...
  const valueColorCar = updateCarEl.childNodes[2].firstChild as HTMLElement;
...
export async function resetCar(id: number) {
  ...
  const resetCarEl = document.querySelector(`.garage-list__element[data-id="${id}"]`) as HTMLElement;
  const startButtonRace = document.querySelector(`.button-start-race[data-id="${id}"]`) as HTMLInputElement;
  startButtonRace.disabled = false;
  const resetButtonRace = document.querySelector(`.button-reset[data-id="${id}"]`) as HTMLInputElement;
  resetButtonRace.disabled = true;
  const valueColorCar = resetCarEl.childNodes[2].firstChild as HTMLElement;
...
export async function raceCars() {
  ...
  start.disabled = true;
  const idsCar = [] as Array<number>;
  cars.forEach(car => {
    idsCar.push(Number(car.getAttribute('data-id')));
  });
  const detailSet = ( await Promise.all(idsCar.map(itemId => startCar(itemId))) as Array<number>).map(el => el == null || el == undefined ? 99 : el);
