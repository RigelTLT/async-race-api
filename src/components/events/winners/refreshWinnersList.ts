...
async function refreshCountAndPage() {
  ...
  const numberPage = Number(elmentPage.innerHTML);
...
export async function changehWinnersListPage(data: IparamsSortWinners) {
  ...
  let listLength = 0;
  if (winners.length - 1 < listPageCar.length - 1) {listLength = winners.length;} else {listLength = listPageCar.length;}
...
export function collectingTableInformation() {
  ...
  const page = (document.querySelector('.number-page__winners') as HTMLElement).innerHTML;
  let sort = 'id';
  let order = 'ASC';
  ...
  const data = { _page: page, _limit: '10', _sort: sort, _order: order };
  return data;
}
